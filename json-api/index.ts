...
export type PostApi<TInput, TOutput> = (
  input: TInput,
  context: RequestContext
) => Promise<Response<TOutput>>;

// withJsonGet converts the output from a handler into JSON.
export const withJsonGet = <TOutput>(
  handler: GetApi<TOutput>
...
export const withJsonPost = <TInput, TOutput>(
  handler: PostApi<TInput, TOutput>
): APIGatewayHandler => async (
  event: APIGatewayProxyEventV2
): Promise<APIGatewayProxyStructuredResultV2> => {
